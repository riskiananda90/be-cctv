...
// API Configuration
const API_BASE = "https://cctvbandaaceh.netlify.app/.netlify/functions/api";
...
function addMarkersToMap() {
  ...
  cctvData.forEach((cctv) => {
    ...
    const marker = L.marker([cctv.lat, cctv.lng], { icon: customIcon }).addTo(
      map
    );
...
function renderCctvList() {
  ...
  if (!authToken) {
    listContainer.innerHTML =
      '<div style="text-align: center; padding: 20px; color: #666;">Silakan login untuk melihat daftar CCTV</div>';
    return;
  }
  ...
  listContainer.innerHTML = filteredData
    .map(
      (cctv) => `
                <div class="cctv-item" onclick="selectCctv('${
                  cctv.id
                    ...
                    <span class="cctv-status status-${cctv.status}">
                        ...
                        ${
                          cctv.status === "online" && cctv.streamUrl
                            ? '<span class="stream-indicator stream-live"></span>'
                            : ""
                        }
                    </span>
                    ${
...
function selectCctv(id) {
  ...
  if (cctv) {
    ...
    const marker = markers.find(
      (m) => m.getLatLng().lat === cctv.lat && m.getLatLng().lng === cctv.lng
    );
    if (marker) {
      marker.openPopup();
    }
...
function showAddCctvModal() {
  if (!currentUser || currentUser.role !== "admin") {
    showNotification(
      "Akses ditolak! Hanya admin yang dapat menambah CCTV.",
      "error"
    );
    return;
  }
...
function editCctv(id) {
  if (!currentUser || currentUser.role !== "admin") {
    showNotification(
      "Akses ditolak! Hanya admin yang dapat mengedit CCTV.",
      "error"
    );
    return;
  }
...
function playVideo(id) {
  ...
  document.getElementById(
    "videoTitle"
  ).textContent = `🔴 Live Stream - ${cctv.name}`;
...
function renderFilteredList(data) {
  ...
  countInfo.style.cssText =
    "padding: 10px; font-size: 0.85rem; color: #666; text-align: center;";
...
setTimeout(() => {
  if (!authToken) {
    showNotification(
      "🏙️ Selamat datang di Sistem CCTV Monitoring Banda Aceh! Silakan login untuk mengakses.",
      "info"
    );
  } else {
    showNotification(
      "🏙️ Selamat datang kembali di Sistem CCTV Monitoring Banda Aceh!",
      "info"
    );
  }
}, 1000);
...
function logPerformance() {
  ...
  console.log(
    `✅ Online CCTV: ${cctvData.filter((c) => c.status === "online").length}`
  );
  console.log(
    `❌ Offline CCTV: ${cctvData.filter((c) => c.status === "offline").length}`
  );
